...
`ondewo-sip-client-cpp` is the C++ gRPC client library for the
[ONDEWO SIP API](https://github.com/ondewo/ondewo-sip-api) - ONDEWO's SIP Telephony service. It is a
...
| `ondewo-sip-api/`                   | submodule - the `.proto` source of truth                                 |
...
C++ has no package registry, so every [GitHub release](https://github.com/ondewo/ondewo-sip-client-cpp/releases)
carries the built package as an asset: `ondewo_sip_client-<version>-<platform>.tar.gz`, plus a
...
archive=ondewo_sip_client-${version}-${platform}.tar.gz

gh release download "${version}" --repo ondewo/ondewo-sip-client-cpp --pattern "${archive}*"
...
```cmake
find_package(ondewo_sip_client CONFIG REQUIRED)
target_link_libraries(my_app PRIVATE ondewo::ondewo_sip_client)
```

```shell
cmake -S . -B build -DCMAKE_PREFIX_PATH=/opt/ondewo_sip_client-${version}-${platform}
```
...
set(ONDEWO_LIBRARY_NAME ondewo_sip_client CACHE STRING "" FORCE)
...
target_link_libraries(my_app PRIVATE ondewo_sip_client)
```
...
```shell
git clone https://github.com/ondewo/ondewo-sip-client-cpp.git
cd ondewo-sip-client-cpp
cmake -S . -B build -DCMAKE_BUILD_TYPE=Release \
  -DONDEWO_LIBRARY_NAME=ondewo_sip_client \
...
```shell
git clone https://github.com/ondewo/ondewo-sip-client-cpp.git   ## Clone repository
cd ondewo-sip-client-cpp                                        ## Change into repo directory
...
generated header you need (`api/ondewo/sip/<file>.grpc.pb.h`) to keep compile times down.
...
int main() {
  ...
  auto channel = grpc::CreateChannel("sip.ondewo.com:443", grpc::SslCredentials({}));

  // Replace Services/Request/Response with the service you need - see the API documentation.
  auto stub = ondewo::sip::Services::NewStub(channel);

  grpc::ClientContext context;
  context.AddMetadata("authorization", "Bearer " + std::string(getenv("ONDEWO_TOKEN")));

  ondewo::sip::Request request;
  ondewo::sip::Response response;
...
docker run \
  -v $(pwd):/input-volume \
  -v $(pwd):/output-volume \
  ondewo-cpp-proto-compiler ondewo-sip-api ondewo ondewo_sip_client
```
...
- In the output volume it deletes only what it owns - `api/`, `include/ondewo_sip_client/`,
  `lib/libondewo_sip_client.a` and `lib/cmake/ondewo_sip_client/` - so a proto that was renamed or
...
- `check_build` asserts that every `.proto` under `ondewo-sip-api/ondewo` produced a matching `.pb.h`.
- `smoke_test` writes a throwaway project that does `find_package(ondewo_sip_client CONFIG REQUIRED)`,
  includes `public-api.h` and links `ondewo::ondewo_sip_client`, then compiles and runs it. That is the
